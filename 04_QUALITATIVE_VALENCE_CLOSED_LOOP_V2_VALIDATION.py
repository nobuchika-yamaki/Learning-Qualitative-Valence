...
class DelayedActionAgencyEstimator:
    ...
    def observe(
        ...
        evidence = 0.55 * prior_adv[best_d] + 0.45 * current_adv
        score = float(sigmoid(6.0 * evidence))
...
class EmbodiedAgent:
    def __init__(
        ...
        self.agency = DelayedActionAgencyEstimator(max_delay=7, seed=seed ^ 0xA631)
    ...
    def vulnerability(self, state: Optional[AgentState] = None) -> float:
        ...
        return float(
            0.35 * (1.0 - s.integrity)
            + 0.25 * s.fatigue
            + 0.25 * (1.0 - s.stability)
            + 0.15 * (1.0 - s.energy)
        )
    ...
    def components(self, event: Event, pain: float, agency_score: float) -> Dict[str, Any]:
        ...
        avoidance_pressure = float(sigmoid(
            1.65 * danger + 1.10 * pain - 0.85 * comfort - 0.55 * controllability
            + 0.55 * dm + 0.25 * (1.0 - self.state.integrity) - 0.05
        ))
        fixed_q = clip01(
...
def run_robustness_battery(
    ...
    for setting in robustness_settings():
        for morph_name, episode in itertools.product(morphologies, range(episodes)):
            ...
            common_seed = deterministic_seed(base_seed, "robustness", setting["setting"], morph_name, episode)
            tasks.append({
                "cfg": asdict(cfg), "condition": "Full_Learned_Q", "episode": episode,
    ...
    for result in results:
        rows.append({**result["meta"], **result["summary"]})
    raw = pd.DataFrame(rows)
    if raw.empty:
        return raw
    return raw.groupby("robustness_setting", as_index=False).agg(
...
def main() -> None:
    ...
    if args.resume and all(path.exists() for path in confirmatory_paths.values()):
        ...
        prospective_df = pd.DataFrame(prospective_rows)
